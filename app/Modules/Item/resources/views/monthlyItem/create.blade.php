...
@section('body')
    ...
    <div class="col-xl-12">
        <ol class="breadcrumb page-breadcrumb">
            ...
            <li class="breadcrumb-item active">@lang('Item::label.MONTHLY') @lang('Item::label.ITEM') @lang('Item::label.SETUP')</li>
            <li class="position-absolute pos-top pos-right d-none d-sm-block"><span class="js-get-date">Sunday, April 12,
                    2020</span></li>
        </ol>
        <div class="subheader">
            <h1 class="subheader-title">
                <i class="fal fa-th-list text-primary"></i>@lang('Item::label.MONTHLY') @lang('Item::label.ITEM') @lang('Item::label.SETUP')
            </h1>
        </div>
        <div id="panel-1" class="panel">
            <div class="panel-hdr">
                <h2>
                    @lang('Item::label.MONTHLY') @lang('Item::label.ITEM') @lang('Item::label.SETUP') <span class="fw-300"></span>
                </h2>
                            ...
                            <div class="card-body">
                                ...
                                <div class="row clearfix">
                                    ...
                                    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                                        ...
                                        <form action="{{ route('store.monthly.item.setup') }}" id="monthlyItem"
                                            method="POST">
                                            @csrf
                                            <div class="table-responsive">
                                                ...
                                                <td><button id="submitBtn"
                                                        class="btn btn-primary btn-sm generate-button float-right mb-3 mr-3"
                                                        onclick="validateForm();">save</button></td>
                                        </form>
                                    </div>
                                </div>
                            </div>
    ...
    <script>
            ...
            $(document).on('change', ".search-item", function() {
                ...
                if ($("#class_id").val() != 0 && $("#academicYearId").val() != 0 && $("#months").val() !=
                    ...
                    $('#yajraTable').DataTable({
                        ...
                        iDisplayLength: 50,
                        "ajax": {
                            "url": "{{ route('monthly.item.setup.index') }}",
                            "data": function(e) {
            ...
            if ($('#chkbxAll').is(':checked')) {
                ...
                $(".allCheck").each(function(index) {
                    ...
                    if ($("#checkSection_" + key)[0].checked == true) {
                        ...
                        $("#allMonthlyItemId_" + key).prop('checked', false);
                    }
                });
            }
        }
        ...
        function unCheck(id) {
            if ($('#' + id).is(':not(:checked)')) {
                ...
                $("#allMonthlyItemId_" + checkboxId[1]).prop('checked', false);
            }
        }
        ...
        function unCheckMonthlyItem(id) {
            if ($('#' + id).is(':not(:checked)')) {
                var checkboxId = id.split("_");
            }
        }

        function checkMonthlyItem(id) {
            if ($('#allMonthlyItemId_' + id).is(':checked')) {
                if ($('#checkSection_' + id).is(':not(:checked)')) {
                    alert('Please Select an Item');
                    $('#allMonthlyItemId_' + id).prop("checked", false);
                }
            }
        }
    </script>
