<div id="panel-1" class="panel">
    <div class="panel-hdr">
        <h2>
            <span class="fw-300"><i>@lang('Contact::label.SUPPLIER') @lang('Item::label.COUNTRY') @lang('Item::label.WISE') @lang('Item::label.PRICING')</i></span>
        </h2>
                            ...
                            <tbody>
                                @if (!$itemArr->isEmpty())
                                    @foreach ($itemArr as $key => $item)
                                        @php
                                            $thisAmount = DB::table('supplier_pricing')->where('product_id',$item->id)->where('dealer_id', $id)->value('price');
                                        @endphp
                                        <tr>
                                            <td>{{ $loop->index + 1 }}</td>
                                            <td>{{ $item->name }}</td>
                                            <td>
                                                {!! Form::text('price[' . $item->id . ']', ($thisAmount ? $thisAmount : null), [
